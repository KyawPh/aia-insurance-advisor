...
async function main() {
  ...
  if (dryRun) {
    ...
    log('⚠️  Running in EXECUTE mode. Changes will be applied!', 'red');
  }
  ...
  try {
    ...
    // Perform cleanup
    await cleanupAllUsers(users, dryRun);
    
    if (dryRun) {
      log(`\n💡 To apply these changes, run: node analyze-and-cleanup-users-auto.js --execute`, 'cyan');
    } else {
      log(`\n✅ All users have been converted to free plan and cleaned up!`, 'green');
    }
