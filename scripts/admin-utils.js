...
// Bulk update free user quota
export async function bulkUpdateFreeUserQuota() {
  const OLD_QUOTA = 5;
  const NEW_QUOTA = 50;
  
  try {
    console.log(chalk.yellow('\nAnalyzing free trial users...'));
    ...
    console.log(chalk.yellow(`Users with old quota (5): ${usersToUpdate}`));
    ...
    console.error(chalk.red('Error updating quotas:'), error);
  }
}
